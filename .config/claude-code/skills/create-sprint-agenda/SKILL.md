...
**推移表は議事録にない。** 前回の振り返りダッシュボード（Artifact）のデータセクションが正本なので、そちらを読む。
...
**過去 4 行は前回のダッシュボードからそのまま引き写す。数え直さない。**
当時の Milestone 構成と status は既に変わっており、今から再集計すると過去の数字が動いて推移が意味を失う。

ダッシュボードが無い回（この形式の導入前、2026-09-17 以前）は、過去 5 件の議事録を `searchPosts` で引き、
...
- 推移表の過去 4 行は前回のダッシュボードから引き写し、再集計しない（Step 2）
