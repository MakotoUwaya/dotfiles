...
Redmine の折りたたみは `{{collapse}}` マクロを使う。**閉じタグは `{{/collapse}}` ではなく `}}` 単体**。

```
{{collapse(表示ラベル)

折りたたまれる内容（Markdown 記法可）

}}
```

- 開始: `{{collapse(ラベルテキスト)`（閉じカッコ `}}` は付けない）
- 終了: `}}` のみ（行頭に単独で記述）
- `{{collapse(ラベル)}}...{{/collapse}}` は **誤り**。この書き方では折りたたみが正しく動作しない
