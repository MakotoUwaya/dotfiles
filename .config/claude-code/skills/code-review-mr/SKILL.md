...
- **1 コメント = 1 話題**。複数の指摘を 1 つのコメントにまとめない（まとめコメント禁止）
...
- **修正対象行に対してインラインドラフトノートで投稿する**（一般コメントではなく、差分の該当行に紐づける）
- MR の差分に含まれないファイルについてのみ一般ドラフトノートとして投稿する
...
- 1 コメント = 1 話題（まとめコメント禁止）
- 修正対象行に対してインラインドラフトノートで投稿する（差分外のファイルのみ一般ドラフト）
- コード修正が必要な場合は、修正案（コード例）を必ず記載する
