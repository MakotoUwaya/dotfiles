...
**`残タスク.md` を読む前に pull する。順序が逆だと pull は防御ではなく喪失の供給源になる。** 読んでから pull すると、pull が持ち込んだもう一方の機の追記を、Step 5 の全体書き換えが静かに消す。git は成功し、競合も起きず、状態ファイルは `ok` のままなので、失われた行に気づく経路が無い。

解決からコマンドまでを1回の `Bash` 呼び出しで完結させる:
...
**この pull が失敗したら、その場で中断してユーザーに報告する。`残タスク.md` の読み込み・書き換えには進まない。** 失敗したまま書き込むと、古いベースの上に全体書き換えをかけることになり、もう一方の機の追加項目を丸ごと消す。作業ツリーが汚れている・同期が競合で止まっている等が典型なので、`/vault-sync-resolve` を案内する。

### 4. 既存残タスク.md との突合

- `Read` で既存の残タスク.md を読み込む（**読んだ内容は Step 5 の再確認で突き合わせるため、そのまま保持しておく**）
...
pull を Read の前に移しても窓は消えない。定期同期は 5 分ごとに作業ツリーを書き換え、ユーザーが Obsidian で `残タスク.md` を開いて編集していることもある。Step 4 の突合と削除候補の提示でユーザーとやり取りすれば、Read と Write の間は分単位で開く。

**`Write` の直前に `残タスク.md` を `Read` し直し、Step 4 で読んだ内容と一致するか確認する。**

**判定は HEAD ではなく内容で行うこと。** HEAD 比較にしてはいけない理由が 2 つある。

- ヴォールトは 5 分ごとに自動コミットされ、Obsidian で手編集しただけでも新しいコミットが載る。Read と Write の間に HEAD が動くのは異常ではなく**日常**であり、HEAD 判定は毎回のように発火して無視されるようになる
- いちばん起こりやすいのは「ユーザーが Obsidian で `残タスク.md` を開いて編集している最中にこのスキルが走る」ケースで、これは**未コミットの作業ツリー変更**なので HEAD は動かない。内容比較なら定期同期の pull・他セッション・Obsidian の手編集の 3 つを 1 つの検査で捕まえられる

一致すればそのまま 5-b へ進む。

**一致しない場合も、分類をやり直す必要はない。** 失われかけているのは「自分が Read した後に入った行」だけで、Step 3 の分類と Step 4 の突合の成果（組み立て済みの本文）は無傷で残っている。捨てるものは何もない:

1. 差分（自分が Read した後に入った行）を特定する
2. Step 3 の判定基準でどのセクションに属するかを決める
3. 組み立て済みの本文に足したうえで 5-b に進む

**差分が「行の削除」だった場合は自動で判断しない。意図的な削除か取りこぼしかをユーザーに確認する。** 機械的に和集合を取ると、ユーザーが意図して消した項目を復活させてしまう（`/vault-sync-resolve` の Step 4「どちらかの側で行が消えている場合」と同じ扱い）。

差分が大きい、あるいは由来が判断できない場合に**限り**、中断してユーザーに提示する。

#### 5-b. Write

`Write` ツールで全体を書き換える（部分編集ではなく完全書き換えのほうが構造が崩れにくい）。`file_path` には 3.5 で解決した `$VAULT` と同じ解決順で得た絶対パス文字列を渡す（`Write` はシェル変数展開をしないため、`$VAULT` という文字列のままでは渡さないこと）。
