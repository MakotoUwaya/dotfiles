...
#nullable enable
// generate-report.cs — Generate a self-contained HTML report from audit results.
//
...
//
// Usage:
//     dotnet script generate-report.cs -- --workspace <dir> [--output <path>] [--template <path>]
...
for (int i = 0; i < Args.Count; i++)
{
    switch (Args[i])
    {
        case "--workspace" when i + 1 < Args.Count:
            workspace = Args[++i];
            break;
        case "--output" when i + 1 < Args.Count:
            output = Args[++i];
            break;
        case "--template" when i + 1 < Args.Count:
            template = Args[++i];
            break;
    }
}
...
{
    Console.Error.WriteLine("ERROR: --workspace is required");
    Console.Error.WriteLine("Usage: dotnet script generate-report.cs -- --workspace <dir> [--output <path>] [--template <path>]");
    return 1;
}

if (!Directory.Exists(workspace))
{
    Console.Error.WriteLine($"ERROR: Workspace not found: {workspace}");
    return 1;
}

if (string.IsNullOrEmpty(output))
{
    output = Path.Combine(workspace, "skill-audit-report.html");
}

// ── Load JSON files ─────────────────────────────────────────────────────────
JsonElement? LoadJson(string path)
{
    ...
    {
        Console.Error.WriteLine($"WARNING: Could not load {path}: {ex.Message}");
        return null;
    }
}

var auditReport = LoadJson(Path.Combine(workspace, "audit-report.json"));
var portfolio = LoadJson(Path.Combine(workspace, "portfolio-analysis.json"));
var proposals = LoadJson(Path.Combine(workspace, "improvement-proposals.json"));
var manifest = LoadJson(Path.Combine(workspace, "skill-manifest.json"));
var history = LoadJson(Path.Combine(workspace, "health-history.json"));
...
string jsonData;
{
    ...
    void WriteProperty(Utf8JsonWriter w, string name, JsonElement? element)
    {
    ...
    WriteProperty(writer, "audit_report", auditReport);
    WriteProperty(writer, "portfolio_analysis", portfolio);
    WriteProperty(writer, "improvement_proposals", proposals);
    WriteProperty(writer, "skill_manifest", manifest);

    // health_history defaults to empty array if null
    writer.WritePropertyName("health_history");
    if (history.HasValue)
    {
        history.Value.WriteTo(writer);
    }
    else
    {
...
string? FindTemplate()
{
    // Auto-detect from ../assets/report_template.html relative to skill dir
    var skillDir = "/home/m-uwaya/.claude/skills/skill-auditor";
    var templatePath = Path.Combine(skillDir, "assets", "report_template.html");
    if (File.Exists(templatePath)) return templatePath;
    return null;
}

string? templatePath2 = template ?? FindTemplate();
string html;

if (templatePath2 != null && File.Exists(templatePath2))
{
    html = File.ReadAllText(templatePath2);
}
else
{
    html = BuiltinTemplate();
}

// ── Inject data and write output ────────────────────────────────────────────
html = html.Replace("/*__EMBEDDED_DATA__*/", $"const REPORT_DATA = {jsonData};");
...
Console.WriteLine($"Report generated: {output}");
return 0;
...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof REPORT_DATA === 'undefined') {
    document.body.textContent = 'No data loaded. Run the audit first.';
    return;
  }
  ...
  const summaryItems = [
    { value: health.overall_score || '?', label: 'Portfolio Health', badgeClass: healthBadge },
    { value: health.routing_accuracy_avg != null ? (health.routing_accuracy_avg * 100).toFixed(0) + '%' : '?', label: 'Avg Accuracy' },
    { value: String(health.competition_conflicts || 0), label: 'Conflicts' },
    { value: String(health.coverage_gaps || 0), label: 'Gaps' },
    { value: String((portfolio.attention_budget || {}).total_tokens || '?'), label: 'Total Desc Tokens' }
  ];
  ...
  function renderCard(r) {
    ...
    for (const inc of (r.incidents || []).slice(0, 5)) {
      ...
      incidentsHtml += '<div class="incident ' + cls + '"><strong>' + esc(inc.verdict) + '</strong>: ' + esc(inc.detail) + '<br><em>"' + esc((inc.user_message || '').substring(0, 100)) + '"</em></div>';
    }
  ...
  if (matrix.length) {
    ...
    for (const m of matrix) {
      ...
      matrixHtml += '<tr>' +
        ...
        '<td><span class="matrix-cell ' + relClass + '"></span> ' + esc(m.relationship) + '</td>' +
    ...
    document.getElementById('competition-matrix').textContent = 'No competition conflicts detected';
  }
  ...
  if (budget.length) {
    ...
    document.getElementById('attention-budget').textContent = 'No attention budget data';
  }
  ...
  if (patches.length) {
    ...
    for (const p of patches) {
      ...
      patchHtml += '<div class="card" style="margin-bottom:1rem">' +
        '<div class="card-header">' +
          ...
          '<span class="badge ' + prioClass + '">' + esc(p.priority) + '</span>' +
        '</div>' +
    ...
    document.getElementById('patches').textContent = 'No patches proposed';
  }
  ...
  if (gaps.length) {
    ...
    document.getElementById('coverage-gaps').textContent = 'No coverage gaps detected';
  }
  ...
  if (newSkills.length) {
    ...
    document.getElementById('new-skills').textContent = 'No new skills suggested';
  }
});
</script>
</body>
